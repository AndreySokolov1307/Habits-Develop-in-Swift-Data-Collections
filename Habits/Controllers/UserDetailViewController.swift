...
class UserDetailViewController: UIViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        setupUI()
        userDetailView.collectionView.register(UICollectionViewListCell.self, forCellWithReuseIdentifier: "HabitCount")
        userDetailView.collectionView.register(NamedSectionHeaderView.self, forSupplementaryViewOfKind: SectionHeader.kind.identifier, withReuseIdentifier: SectionHeader.reuse.identifier)
        dataSource = createDataSource()
        userDetailView.collectionView.dataSource = dataSource
        
        update()
        
        imageRequestTask = Task {
            if let image = try? await ImageRequest(imageID: user.id).send() {
                self.userDetailView.profileImageView.image = image
            }
    ...
    private func setupUI() {
        userDetailView.userNameLabel.text = user.name
        userDetailView.bioLabel.text = user.bio
        userDetailView.backgroundColor = user.color?.uiColor ?? .white
    ...
    private func createDataSource() -> DataSourceType! {
        let dataSource = DataSourceType(collectionView: userDetailView.collectionView) {
        ...
        dataSource.supplementaryViewProvider = { (collectionView,
            ...
            let header = collectionView.dequeueReusableSupplementaryView(ofKind: SectionHeader.kind.identifier,
